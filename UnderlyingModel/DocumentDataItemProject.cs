...
namespace UnderlyingModel
{
    ...
    public class DocumentDataItemProject
    {
        ...
        public AssemblyLocation asmLocation;
        ...
        [NonSerialized()]
        [XmlIgnore()]
        private readonly List<IDataItem> _allItems;
		...
		public List<IDataItem> GetFilteredItems (Presence api, Presence docs)
		{
			IEnumerable<IDataItem> items = new List<IDataItem>(_allItems);
        ...
        public static DocumentDataItemProject Load(String path)
        {
            ...
            //
            // for some reason if file is moved, fix docPath
            //
            String loadedDir = Path.GetDirectoryName(path);
            String docParentDir = Path.GetDirectoryName(project.docPath);

        	if (!docParentDir.Equals(loadedDir))
        		project.docPath.Replace(docParentDir, loadedDir);
        ...
        public void Save()
        {
            String parentDir = Path.GetDirectoryName(docPath);
