...
namespace UnderlyingModel.Tests
{
	...
	public class DLLtests
	{
		...
		public void Init()
		{
			_assemblyDataItemSet = AssemblyDataBuilder.BuildAssemblyDataItemsMonoCecil(_assemblyLocation);
		...
		{
			bool success = _assemblyDataItemSet.Items.Any(item => item.UniqueName.Equals(st));
			Assert.IsTrue(success);
		}

		private void VerifyAbsentFromAssembly(string st)
		{
			bool contains = _assemblyDataItemSet.Items.Any(item => item.UniqueName.Equals(st));
